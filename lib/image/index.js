...
class Image {
  ...
  read( buffer, offset, length, position, callback ) {
    ...
    var fromGTEOffset = ( fromGDE * this.gtBytes ) + fromGTE
    var fromGrainSector = this.gt.readUInt32LE( fromGTEOffset * VMDK.GTE_SIZE )

    var offsetInGrain = ( fromLBA * VMDK.SECTOR_SIZE ) - position
    ...
    // We're still within one grain -> simple read within grain
    if( this.grainBytes - offsetInGrain >= length ) {

      var readPosition = ( fromGrainSector * VMDK.SECTOR_SIZE ) + offsetInGrain

      // Grain has not been allocated (0), or
      // Sector is in a Zeroed-Grain (1);
      // Return a zero-filled buffer
      if( fromGrainSector === 0 || fromGrainSector === 1 ) {
        process.nextTick(() => {
          buffer.fill( 0x00, offset, offset + length )
      ...
      // We have to read from multiple grains
      console.log( 'read:inter-grain', position, length )
      process.nextTick(() => {
...
Image.ReadStream = require( './read-stream' )

Image.createReadStream = function( path, options ) {
  options = options || {}
  options.path = path
  return new Image.ReadStream( options )
}
